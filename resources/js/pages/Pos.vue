<template>
	<div id="gate-out-app">
			...
			<el-col :span="13">
				<el-card>
					...
					<el-row :gutter="10">
						<el-col :span="12">
							<button
								id="btn-karcis-hilang"
								@keydown.enter="formKarcisHilang = true"
								class="my-big-btn"
								@click="formKarcisHilang = true"
							>
								[F10] KARCIS HILANG
							</button>
						</el-col>
						<el-col :span="12">
							<button
								id="btn-buka-manual"
								@keydown.enter="formBukaManual = true"
								class="my-big-btn"
								@click="formBukaManual = true"
							>
								[F11] BUKA MANUAL
							</button>
						</el-col>
				...
				<el-card class="text-center" v-if="showUserInfo">
					...
					<div style="font-size: 16px">
						NIP: {{ user.nip }}<br />
						Dept. : {{ user.department ? user.department.nama : "" }}
					</div>
					<div style="margin: 10px 0 20px 0">
						<el-tag effect="dark" type="success" style="font-size: 20px">
							{{ user.plat_nomor }}
						</el-tag>
					</div>
				</el-card>
		...
		<FormBukaManual
			:show="formBukaManual"
			@close-form="formBukaManual = false"
			@open-gate="openGate"
		/>
		<FormKarcisHilang
			:show="formKarcisHilang"
			@close-form="formKarcisHilang = false"
			@open-gate="openGate"
		/>
	</div>
</template>
...
export default {
	...
	methods: {
			...
			axios
				...
				.then(user => {
					if (user) {
						...
						axios
							.get("api/accessLog/search", { params })
							.then(r => {
		...
		checkTicket() {
			...
			axios
				.get("api/accessLog/search", { params: params })
				.then(r => {
					this.formModel = r.data;
					this.snapshot_in = r.data.snapshot_in;
		...
		submit() {
			...
			axios
				.put("accessLogs/" + this.formModel.id, this.formModel)
				.then(r => {
					this.openGate();
				})
				.catch(e => {
		...
		takeSnapshot() {
			axios
				.get("api/barrierGate/takeSnapshot/" + this.gateOut.id)
				.then(r => {
					this.snapshot_out = r.data.filename;
		...
		getStream() {
			axios
				.post(
					"/barrierGate/testCamera/" + this.gateOut.id,
					{},
					{ timeout: 1000 }
				)
				.then(r => {
